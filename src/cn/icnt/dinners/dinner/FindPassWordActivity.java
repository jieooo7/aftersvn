...
public class FindPassWordActivity extends Activity {
	...
	public void clickMethod(View v) {
		...
		case R.id.change_password:
			...
			if (canChange) {
				if (!StringUtils.isEmpty(newpwd) && !StringUtils.isEmpty(confirmpwd) && confirmpwd.equals(newpwd)) {
		...
		http.send(HttpRequest.HttpMethod.POST,
				...
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						Log.i("order", responseInfo.result);
						Gson gson = new Gson();
						answer_res = gson.fromJson(responseInfo.result, FindPwdBean.class);
						if (answer_res !=null) {
							if ("10000".equals(answer_res.head.code)) {
								text_answer.setTextColor(getResources().getColor(R.color.white_text));
								answer.setBackgroundColor(getResources().getColor(R.color.change_btn_bg));
								
								text_change_password.setTextColor(getResources().getColor(R.color.tab_font));
								change_password.setBackgroundColor(getResources().getColor(R.color.login_btn_bg));
								canChange=true;
		...
		http.send(HttpRequest.HttpMethod.POST,
				...
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						Gson gson = new Gson();
						change_pwd = gson.fromJson(responseInfo.result, FindPwdBean.class);
						if ("10000".equals(change_pwd.head.code)) {
							ToastUtil.show(FindPassWordActivity.this, "###"+change_pwd.head.msg+"###");
